...
use App\Models\User;
use Auth ;
class 02TvShowsController extends Controller
{
    ...
    //   dd($body_Api);
       function getNm($repeat,$nmep,$input_repeat,$input_nmep,$allpage,$allpagenm,$page,$pagenm)
       {
        ...
        header('Refresh: 1; URL='.url('admincp/autofix/seasons/'.$page.'/'.$pagenm.'/'.$input_repeat.'/'.($input_nmep+1).'/'));
           ...
           if (intval(($allpage-1)) == intval($page)){
       ...
       dd(getNm($body_Api->count_repeat,$body_Api->count_ep,$repeat,$nmep,$body_all->seasons,$body_all->pages,$page,$pagenm));
