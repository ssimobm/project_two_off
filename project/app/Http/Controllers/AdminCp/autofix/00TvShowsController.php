...
use App\Models\User;
use Auth ;
class 00TvShowsController extends Controller
{
 ...
 // end save info movies meta
  ...
  if (!isset($chek)) {
    ...
    if (isset($moviee["postimg"]) && strlen($Seasons->post_img) == 0) {
...
// end save info movies meta
  ...
  public function episode($seasons,$movie)
  {
   ...
   //          ->first() ;
   // }
  $moviee = $apidata->ApiOneAuto(($movie->tmdb??$chek->tmdb_id),"episode",($movie->season??$chek->season_nm),$movie->episode) ;
   if (!isset($chek)) {
   ...
   if (isset($moviee["postimg"]) && strlen($DbTVshows->post_img) == 0) {
...
//http://filmoq.md/storage/tvshows/1/0.json
     ...
     for ($i=0; $i <= ($body_Api->seasons-1) ; $i++) {
       $link = "http://filmoq.md/storage/tvshows/".($page??1)."/".$i.".json" ;
       $body = Http::get($link)->body() ;
       $body_Api = json_decode($body);
       // start season
...
// $seasonall[] = [$season["seasons"],$episodes] ;
      ...
      if (intval($body_Api->pages) >= intval($page)){
        header('Refresh: 1; URL='.url('/autofix/seasons/'.($page+1)));
      }
// dd($body_Api);
      //return $body ;
    }
