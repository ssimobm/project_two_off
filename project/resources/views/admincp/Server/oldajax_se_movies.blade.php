...
<div class="tab-pane" id="messages-fill" role="tabpanel" aria-labelledby="messages-tab-fill">
  ...
  @foreach ($comments as $key => $value)
    @php
      $simoo = $value->where('user_id',  $value->user_id)->first()->user->name;
    @endphp
    <div class="border border-light p-1 mb-1">
    <div class="media">

    <div class="media-body">
        <h5 class="m-0">{{$simoo}}</h5>
        <p class="text-muted"><small>about {{ $value->created_at->diffForHumans() }}</small></p>

    </div>
    </div>
    <p>{{$value->comments}}</p>

  <button class="btn1" ids='{{$value->id}}'>Prepend text</button>
  </div>

  <div class='comments_{{$value->id}}'></div>

  @endforeach
  <div id="comments"></div>
  <button class="btn1" ids='sim0'>Prepend text</button>
  </div>
  <div class='comments_sim0'></div>
 ...
 $('#loader1').hide();
    ...
    $(document).ready(function(){
     $(".btn1").click(function(){
      var comment_id = $(this).attr("ids");
       var html2 = '';
       html2 += '<div class="comment-area-box simo mt-2 mb-3" id="comment_'+ comment_id +'">';
       html2 += '<span class="input-icon"> <textarea rows="3" class="commento_'+comment_id+' form-control" placeholder="Write something..."></textarea> </span> <div class="comment-area-btn"> <div> <button type="submit" class="commento btn btn-sm btn-light text-black-50 btn-dark waves-effect waves-light" ids="'+comment_id+'">Post</button> </div> </div> ';
       html2 += '</div>';

      $('.comments_'+comment_id).append(html2);
    ...
    $(".commento").click(function(){
      simo++;
      var comment_id = $(this).attr("ids");
    var comment =  $(".commento_"+comment_id).val();
