...
<div class="container-fluid">
      <div class="owl-carousel owl-theme">
        @foreach ($Movies as $k => $v)
          ...
          <div class="item">
  <a href='{{ url('movies') }}/{{ $v->slug }}'  class="card filmoq-box getlink" >
    {{-- <div class="bg-dark text-white">
    {{Str::limit($v->Name_tv, 20, '...')}}
    </div> --}}
    <img src="{{ url('storage/images') }}/{{$v->tvdata->where('simokey', 'Movie_Postimg')->first()->simovalue}}
  ...
  @foreach ($Tvshows as $k => $v)
    ...
    <div class="col-lg-2 col-md-3 col-sm-3 col-6" id="#title_{{ $v->id }}" title="{{$v->title}}">
     @if (isset(Auth::user()->id))
      ...
      <a href='{{ url('tvshows') }}/{{ $v->slug }}' class="card filmoqbox filmoq-box getlink" >
        {{-- <div class="bg-dark text-white">
        {{Str::limit($v->Name_tv, 20, '...')}}
        </div> --}}
...
@php
$link = $v->where('id', $v->id)->first()->tv->slug ;
$id_ep = $v->where('id', $v->id)->first()->tv->id ;
@endphp
         <div class="col-lg-3 col-md-3 col-sm-3 col-6">
 @if (isset(Auth::user()->id))
  <div class="bg-dark text-white">

    <button type="button" class="notify btn btn-danger" id="{{ $id_ep }}" data-toggle="tooltip" data-placement="top" title="" titlee="{{$v->Name}}" data-original-title="Add Notification">
    <i class="fa fa-bell"></i>
      </button>
     <button type="button" class="watchcon btn btn-danger" id="{{ $v->id }}" data-toggle="tooltip" data-placement="top" title="" titlee="{{$v->Name}}" data-original-title="Add watch later">
    <i class="fas fa-clock"></i>
     </button>
   </div>
   @endif
         <a href='{{ url('tvshows') }}/{{ $link }}/season/{{ $v->Sea_Nm }}/episode/{{ $v->Ep_Nm }}'  class="card filmoqbox filmoq-box getlink" >
         {{-- <div class="bg-dark text-white">
         {{Str::limit($v->Name_tv, 20, '...')}}
         </div> --}}
         <img src="{{ url('storage/images') }}/{{$v->Postimg}}" class="img-ep mx-auto d-block" alt="...">
         <div class="filmoq filmoq-blue filmoq-left">Season {{$v->Sea_Nm}} </div>
         <div class="filmoq filmoq-blue filmoq-right">{{$v->Ep_Nm}} </div>
         <div class="card-body">
 <h5 class="card-title">
 {{-- Episode {{$v->Ep_Nm}} --}}
 {{Str::limit($v->Name_tv, 20, '...')}}
...
<button type="button" class="btn btn-block btn-outline-blue btn-sm waves-effect waves-light  mb-4">Load More</button>
 ...
 <div class="bg-dark text-white">
   ...
   <button type="button" class="favorit btn btn-danger" id="{{ $v->id }}" data-toggle="tooltip" data-placement="top" titlee="{{$v->title}}" title="" titlee="{{$v->title}}" data-original-title="Add Favorit">
   <i class="far fa-heart"></i>
     </button>
    <button type="button" class="watchlater btn btn-danger" id="{{ $v->id }}" data-toggle="tooltip" data-placement="top" title="" titlee="{{$v->title}}" data-original-title="Add watch later">
   <i class="fas fa-clock"></i>
    </button>
  </div>
  @endif
