...
<div class="col-12">
  <div class="d-flex justify-content-between">
    ...
    <ul class="action-bar-player-right nav">
      ...
      <li class="nav-item">
      <a href="{{url('tvshows')}}/{{$Episode->slug}}/season/{{$Seasons->season_nm}}/episode/{{$Episodes->episode_nm - 1}}" type="button" class="btn btn-icon btn-blue mr-1 mb-1 waves-effect waves-light" data-toggle="tooltip" data-placement="top"  title="" data-original-title="Previous Episode"><i class="fas fa-arrow-right"></i></a>
      </li>
      @endif
      @if (isset($Seasons->episode->where('episode_nm', $Episodes->episode_nm + 1)->first()->episode_nm))
      <li class="nav-item">
      <a href="{{url('tvshows')}}/{{$Episode->slug}}/season/{{$Seasons->season_nm}}/episode/{{$Episodes->episode_nm + 1}}" type="button" class="btn btn-icon btn-blue mr-1 mb-1 aves-effect waves-light" data-toggle="tooltip" data-placement="top"  title="" data-original-title="Next Episode"><i class="fas fa-arrow-left"></i></a>
      </li>
      @endif
    </ul>
    </div>
    <div class="p-0">
      ...
      <button type="button" class="favorit btn btn-icon btn-icon rounded-circle btn-blue mr-1 waves-effect waves-light" id="{{ $Episode->id }}" titlee="{{$Episode->title}}" types="tv" data-toggle="tooltip" data-placement="top"  title="" data-original-title="Add Favorit"><i class="far fa-bookmark"></i></button>
      <button type="button" class="notify btn btn-icon btn-icon rounded-circle btn-blue mr-1waves-effect waves-light" id="{{ $Episode->id }}" titlee="{{$Episode->title}}" types="tv" data-toggle="tooltip" data-placement="top"  title="" data-original-title="Add Notification"><i class="fa fa-bell"></i></button>
      </div>
    </div>
...
@if (url()->current() == url('tvshows')."/".$Episode->slug."/season/".$Seasons->season_nm."/episode/".$value->episode_nm)
<a class="simoclist list-group-item active" href="{{url('tvshows')}}/{{$Episode->slug}}/season/{{$Seasons->season_nm}}/episode/{{$value->episode_nm}}">{{$value->episode_nm}}</a>
@else
<a class="simoclist list-group-item" href="{{url('tvshows')}}/{{$Episode->slug}}/season/{{$Seasons->season_nm}}/episode/{{$value->episode_nm}}">{{$value->episode_nm}}</a>
@endif
@endforeach
</div>
</div>
...
<div class="container-fluid  bg-dark">
    ...
    <div class="card card-body">
      ...
      <a href="{{url('tvshows')."/".$Episode->slug}}">{{$Episodes->name}}</a>
